...
$remain=(int)$_SESSION['leave_no'];
$got=40-(int)$_SESSION['leave_no'];
//$result=($total-$num1);
	$chart = new PieChart();

	$dataSet = new XYDataSet();
	$dataSet->addPoint(new Point("Remaining # of leave",$remain));
	$dataSet->addPoint(new Point("Leave got",$got));
	...
	$chart->setTitle("Attendance status");
	$chart->render("generated/demo3.png");
	
	$uid1=$_SESSION['uid1'];
	
	$query1="select * from leave_status where emp_id='$uid1' and status='Accepted'";
	$result1=mysql_query($query1) or die("query failed <br><b>".mysql_error);	
	$num_rows1 = mysql_num_rows($result1);
	
	$query1="select * from leave_status where emp_id='$uid1' and status='Denied'";
	$result1=mysql_query($query1) or die("query failed <br><b>".mysql_error);	
...
//$result=($total-$num1);
	...
	$dataSet = new XYDataSet();
	$dataSet->addPoint(new Point("Accepted Requests",$x1));
	$dataSet->addPoint(new Point("Denied Requests",$x2));
	//$dataSet->addPoint(new Point("Opera (50)", 50));
	//$dataSet->addPoint(new Point("Safari (37)", 37));
	//$dataSet->addPoint(new Point("Dillo (37)", 37));
	//$dataSet->addPoint(new Point("Other (72)", 70));
...
<div id="wrapper">
    ...
    <div class="content_bottom">
     <div class="col-md-8 span_3">
		...
		<h3 class="text-center">Accepeted Vs Denied Requests</h3>
